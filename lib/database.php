...
// update point
function update_point($type = "plus",$account_id,$money){
    $sql = "";
    $money = abs(intval($money));
    if($type == "plus"){
        $sql = "UPDATE `account` SET `a_point` = `a_point` + '{$money}' WHERE `account_id` = '{$account_id}' LIMIT 1";
    }
    else{
        $sql = "UPDATE `account` SET `a_point` = `a_point` - '{$money}' WHERE `account_id` = '{$account_id}' LIMIT 1";
    }
